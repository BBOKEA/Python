...
# if cam.isOpened() == False: #카메라 생성 확인을 위해 추가
#     print ('Can\'t open the CAM(%d)') % (CAM_ID)
#     exit()
#prevTime = 0 #이전 시간을 저장할 변수

def opencamera():  #카메라 오픈시 동작 함수
    webcam = cv2.VideoCapture(0) #카메라 0 웹캠 1 외부 카메라 
    ...
    while webcam.isOpened():
        ...
        #     opencamera() #램 메모리를 줄이기 위한 카운트 변수 선언후 150 사이클 이후 재실행 
           
    
        if cv2.waitKey(5) == 27: #esc로 코드 종료
            break
        if cv2.waitKey(5) == 24: #ctrl+x로 속도측정 소스로 전환
            os.system("CarSpeedCheck.py")
